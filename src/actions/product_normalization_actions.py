...
class ProductNormalizationActions:
    ...
    async def match_or_enqueue(self, invoice_items: list[InvoiceItemEntity]) -> None:
        ...
        fingerprints = [generate_fingerprint(item.description) for item in invoice_items]
    ...
    async def normalize_unmatched_products(self, batch_size: int = 15) -> int:
        ...
        logger.info(f'Found {len(unmatched_items)} unmatched items to check for normalization.')
        ...
        unique_fingerprints = [generate_fingerprint(item.description) for item in items_to_normalize]
        ...
        for result in normalized_results:
            logger.info(f'Processing normalized result: {result.model_dump(mode="json") if hasattr(result, "model_dump") else result}')
            ...
            if not existing:
                canonical = CanonicalProductEntity(
                    ...
                    fingerprint=fp,
                    ai_metadata=result.model_dump(mode='json') if hasattr(result, 'model_dump') else {},
                )
                await self._canonical_repository.create(canonical)
                created_count += 1
            else:
                logger.info(f'Canonical product for fingerprint {fp} already exists, skipping creation.')

        logger.info(f'AI normalization batch completed. Created {created_count} canonical products.')
    ...
    async def match_unmatched_items(self, batch_size: int = 100) -> int:
        ...
        logger.info(f'Found {len(unmatched_items)} unmatched items to match with existing database catalog.')

        fingerprints = [generate_fingerprint(item.description) for item in unmatched_items]
        canonical_products = await self._canonical_repository.find_by_fingerprints(
            fingerprints
        )
        canonical_map = {p.fingerprint: p for p in canonical_products}
    ...
    async def remove_user_override(self, user_id: int, canonical_product_id: int) -> None:
        override = await self._override_repository.find_by_user_and_product(
            user_id, canonical_product_id
        )
        if override:
